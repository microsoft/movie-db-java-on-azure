...
1. Clone [elk-acs-kubernetes](https://github.com/Microsoft/elk-acs-kubernetes) repo.
2. Install Helm. [Quickstart Guide](https://github.com/kubernetes/helm/blob/master/docs/quickstart.md)
3. Take a note of GROUP_SUFFIX you set previously in `deployment/config.json` e.g. "123456". Data app is deployed to two Kubernetes clusters in resource groups e.g. `devEastUSGroup123456`, `devWestEuropeGroup123456`. So does ELK cluster.
4. Run the following command to get the `loginserver` and `name` of your Azure Container Registry(ACR) instance.

   ```shell
   az acr list
   ```
5. Run the following command to get the password of your ACR instance.
   ```shell
   az acr credential show --name <registry_name>
   ```
6. Update the following fields in `elk-acs-kubernetes/docker/push-images.sh`:

   a. `registry_server` with `loginserver`
   
   b. `registry_username` with `name`
   
   c. `registry_password` with `password`.
7. Update `elk-acs-kubernetes/helm-charts/start-elk.sh` as above as well as `registry_email` with a valid email address.
8. Run the following commands to create storage account in the corresponding resource group e.g. `devEastUSGroup123456`:
   ```shell
   ...
   ```
9. In `elk-acs-kubernetes/helm-charts/config.yml`, update `registry` with `loginserver` and `location` with "eastus".
10. Run the following command to get `dnsPrefix` of ACS instance.
   ```shell
   az acs list --resource-group=devEastUSGroup123456
   ```
11. Run the following command to set the current context for kubectl.
   ```shell
   kubectl config set-context <dnsPrefix>
   ```
12. Run the following command to install tiller to Kubernetes cluster.
   ```shell
   helm init
   ```
13. Run the following command to deploy ELK cluster.
   ```shell
   elk-acs-kubernetes/helm-charts/start-elk.sh
   ```
14. Update `registry` in `elk-acs-kubernetes/helm-charts/filebeat/values.yaml` with registry name and run the following command to install filebeat.
   ```shell
   helm install filebeat
   ```
15. Repeat step 9 to 12 for resource group "devWestEuropeGroup123456".
16. You can now run the following command to launch a proxy at http://localhost:8001/ui to monitor deployment status.
   ```shell
   kubectl proxy
   ```
After deployments finish, access public Kibana endpoint, create index pattern and start to visualize log streams.
